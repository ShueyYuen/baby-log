...
生产镜像已包含 ffmpeg。非 Docker 部署需自行安装 `ffmpeg` / `ffprobe`（如 `apt install ffmpeg`）。上传的视频在后台排队处理（同时只跑 1 路），避免 2C4G 机器被打满。默认：x264 `veryfast`、CRF 28、单线程、`nice 19`、最长边 1280、超过 30fps 降到 30，并生成 `.poster.jpg`。转码失败或超时会保留原片。使用 S3 时，需要转码的视频先只落本地，转码成功后再上传，避免 CDN 把不可播的原片按 `immutable` 缓存一年。进程重启或更新后会从数据库里 `ready=0` 的记录恢复排队（ffmpeg 不支持从半截编码接着跑，会用原片重来）。转码完成前接口不返回播放地址，静态文件也 404，前端只显示封面和「处理中」。
